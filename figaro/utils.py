...
def get_priors(bounds, samples = None, mean = None, std = None, df = None, k = None, a = None, scale = None, probit = True, hierarchical = False):
    ...
    if not hierarchical:
        ...
        if std is not None:
            ...
            if probit:
                out_sigma       = (np.sqrt(np.diag(np.cov(np.atleast_2d([np.median(ev, axis = 0) for ev in probit_samples]).T)))/scale).flatten()#(np.sqrt(all_samples_cov - events_avg_cov)/scale).flatten()
            else:
                out_sigma       = (np.sqrt(np.diag(np.cov(np.atleast_2d([np.median(ev, axis = 0) for ev in samples]).T)))/scale).flatten()#(np.sqrt(all_samples_cov - events_avg_cov)/scale).flatten()
